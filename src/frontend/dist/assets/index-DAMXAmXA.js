...
function App() {
  ...
  const drawOverlayScreen = reactExports.useCallback(
    (ctx, W, H, gs, type) => {
      ctx.fillStyle = type === "victory" ? "rgba(4,12,22,0.92)" : "rgba(2,4,6,0.95)";
      ctx.fillRect(0, 0, W, H);
      ctx.textAlign = "center";
      if (type === "gameover") {
  ...
  const gameLoop = reactExports.useCallback(
    (timestamp) => {
      ...
      if (currentScreen === "start") {
        drawStartScreen(ctx, W, H, timestamp / 1e3);
      ...
      gs.nextNotifIn -= dt;
      if (gs.nextNotifIn <= 0) {
        const txt = NOTIFICATIONS[Math.floor(Math.random() * NOTIFICATIONS.length)];
      ...
      if (alivePlayers.length > 0) {
        const cx = alivePlayers.reduce((s, p) => s + p.x, 0) / alivePlayers.length;
        const cy = alivePlayers.reduce((s, p) => s + p.y, 0) / alivePlayers.length;
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "P1: WASD + F  |  P2: ARROWS + L  |  SURVIVE 5 HOURS"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: screen === "playing" ? "▶ PLAYING" : "PRESS ENTER"
                }
              )
            ]
          }
